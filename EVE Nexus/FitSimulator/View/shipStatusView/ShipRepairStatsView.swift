...
struct ShipRepairStatsView: View {
    ...
    private func calculateTankData(ship: SimShipOutput) -> TankData {
        ...
        let passiveShieldRecharge = calculateShieldRecharge(
            capacity: shieldCapacity, rechargeRate: shieldRechargeRate / 1000.0
        )
    ...
    private func calculateResistances(ship: SimShipOutput) -> (
        ...
        // 计算平均抗性
        let shieldAvgResist =
            (shieldEmResist + shieldThermResist + shieldKinResist + shieldExpResist) / 4.0
        let armorAvgResist =
            (armorEmResist + armorThermResist + armorKinResist + armorExpResist) / 4.0
            ...
            Section {
                VStack(spacing: 2) {
                    ...
                    HStack {
                        ...
                        // 考虑抗性后的有效修复量
                        Text(formatValue(tankData.passiveShield / (1 - tankData.shieldResist)))
                            .fixedSize()
                            .frame(maxWidth: .infinity)
                        Text(formatValue(tankData.shieldRepair / (1 - tankData.shieldResist)))
                            .fixedSize()
                            .frame(maxWidth: .infinity)
                        Text(formatValue(tankData.armorRepair / (1 - tankData.armorResist)))
                            .fixedSize()
                            .frame(maxWidth: .infinity)
                        Text(formatValue(tankData.hullRepair / (1 - tankData.hullResist)))
                            .fixedSize()
                            .frame(maxWidth: .infinity)
                    }
                }
