...
struct ShipFittingModulesView: View {
    ...
    private func batchReplaceModules(flags: [FittingFlag], newTypeId: Int) {
        ...
        let effectQuery = "SELECT effect_id FROM typeEffects WHERE type_id = ?"
        if case let .success(rows) = viewModel.databaseManager.executeQuery(
            effectQuery, parameters: [newTypeId]
        ) {
            for row in rows {
                if let effectId = row["effect_id"] as? Int {
                    effects.append(effectId)
        ...
        if case let .success(rows) = viewModel.databaseManager.executeQuery(
            infoQuery, parameters: [newTypeId]
        ),
            let row = rows.first
        {
            model_name = row["name"] as? String ?? ""
    ...
    // 带折叠按钮的区域头部视图
    private func sectionHeaderWithCollapse(
                    ...
                    if miningAmount > 0 {
                        HStack(spacing: 4) {
                            ...
                            HStack(spacing: 0) {
                                ...
                                .foregroundColor(.secondary)
                                Text(
                                    "\(formatNumber(miningAmount, digits: 2)) m³ + \(formatNumber(miningWasteProbability, digits: 2))%"
                                )
                                .font(.caption)
                                .fontWeight(.semibold)
                                .foregroundColor(.secondary)
