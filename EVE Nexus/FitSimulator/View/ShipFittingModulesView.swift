...
struct ShipFittingModulesView: View {
    ...
    // 检查是否可以安装模块到指定槽位（保留原函数以兼容其他调用）
    private func canInstallModule(typeId: Int, flag: FittingFlag) -> Bool {
        // 从数据库获取装备信息
        var attributes: [Int: Double] = [:]
        var attributesByName: [String: Double] = [:]
        var effects: [Int] = []
        ...
        // 查询装备分组
        let groupQuery = "SELECT groupID FROM types WHERE type_id = ?"
        if case let .success(rows) = viewModel.databaseManager.executeQuery(groupQuery, parameters: [typeId]),
           let row = rows.first,
