...
class Step4 {
    ...
    // 需要应用叠加惩罚的操作类型
    private let stackableOperations: [String] = ["PreMul", "PreDiv", "PostMul", "PostDiv", "PostPercent"]
    ...
    init(databaseManager: DatabaseManager, step3: Step3) {
        ...
        self.step3 = step3
        // 从Step3获取属性默认值缓存
        self.attributeDefaultValueCache = step3.getAttributeDefaultValueCache()
    }
    ...
    /// 获取物品的详细信息
    private func getItemDetails(item: Any) -> (typeId: Int, attributes: [Int: Double], modifiers: [Int: [SimAttributeModifier]]) {
    ...
    /// 将计算结果更新到输出对象
    private func updateOutputWithCachedValues(input: SimulationInput, output: inout SimulationOutput, cache: Cache) {
        // 更新飞船属性
    ...
    private func calculateAttributeValue(
        ...
        // 检查缓存中是否已有计算结果
        if let _ = cache.getValue(itemType: itemType, itemIndex: itemIndex, attributeId: attributeId) {
            return
        }
        ...
        var process = "[AttributeCalc] TypeID：\(typeId)，实例ID：\(instanceIdShort)，属性：\(attrName)(ID: \(attributeId))，基础值：\(baseValue)"
        
        // 如果没有修饰器，直接缓存基础值
        if modifiers.isEmpty {
            cache.setValue(itemType: itemType, itemIndex: itemIndex, attributeId: attributeId, value: baseValue)
            process += "\n  无修饰器，最终值：\(baseValue)"
        ...
        // 按操作类型分组处理修饰器
        let operationTypes = ["PreAssign", "PreMul", "PreDiv", "ModAdd", "ModSub", 
        ...
        // 缓存计算结果
        cache.setValue(itemType: itemType, itemIndex: itemIndex, attributeId: attributeId, value: currentValue)
        ...
        case "PreAssign", "PostAssign":
            ...
            for modifier in finalModifiers {
                ...
                if bestValue == nil {
                    ...
                    bestSource = "\(sourceName)的\(sourceAttrName)(\(modifier.sourceAttributeId)): \(sourceValue)"
    ...
    private func getSourceAttributeValue(
        ...
        if input.ship.instanceId == sourceInstanceId {
            if let value = cache.getValue(itemType: .ship, itemIndex: 0, attributeId: sourceAttributeId) {
                return value
            }
        ...
        if input.character.instanceId == sourceInstanceId {
            if let value = cache.getValue(itemType: .character, itemIndex: 0, attributeId: sourceAttributeId) {
                return value
            }
        ...
        if let (module, moduleIndex) = moduleInstanceMap[sourceInstanceId] {
            if let value = cache.getValue(itemType: .module, itemIndex: moduleIndex, attributeId: sourceAttributeId) {
                return value
            }
        ...
        for (index, module) in input.modules.enumerated() {
            if let charge = module.charge, charge.instanceId == sourceInstanceId {
                if let value = cache.getValue(itemType: .charge, itemIndex: index, attributeId: sourceAttributeId) {
                    return value
                }
        ...
        for (index, drone) in input.drones.enumerated() {
            if drone.instanceId == sourceInstanceId {
                if let value = cache.getValue(itemType: .drone, itemIndex: index, attributeId: sourceAttributeId) {
                    return value
                }
        ...
        if let fighters = input.fighters {
            for (index, fighter) in fighters.enumerated() {
                if fighter.instanceId == sourceInstanceId {
                    if let value = cache.getValue(itemType: .fighter, itemIndex: index, attributeId: sourceAttributeId) {
                        return value
                    }
        ...
        for (index, implant) in input.implants.enumerated() {
            if implant.instanceId == sourceInstanceId {
                if let value = cache.getValue(itemType: .implant, itemIndex: index, attributeId: sourceAttributeId) {
                    return value
                }
        ...
        for (index, skill) in input.skills.enumerated() {
            if skill.instanceId == sourceInstanceId {
                if let value = cache.getValue(itemType: .skill, itemIndex: index, attributeId: sourceAttributeId) {
                    return value
                }
    ...
    /// 获取物品名称
    private func getItemName(typeId: Int) -> String {
        ...
        // 特殊处理角色类型ID
        if typeId == 1373 { // 使用硬编码的角色类型ID 1373
            return "角色"
        }
    ...
    /// 显示属性计算结果，用于调试和验证
    private func displayAttributeCalculationResults(input: SimulationInput, output: SimulationOutput) {
        Logger.info("===== 属性计算结果 =====")
    ...
    private func deduplicateDbuffModifiers(
        ...
        for (effectId, groupModifiers) in dbuffGroups {
            if groupModifiers.count == 1 {
                ...
                if let selected = selectedModifier {
                    ...
                    Logger.info("效果ID \(effectId): 发现 \(groupModifiers.count) 个相同的 dbuff 修饰器，聚合模式: \(aggregateMode)")
                    
                    // 显示所有修饰器的强度
                    for (index, detail) in strengthDetails.enumerated() {
                        let isSelected = detail.modifier.sourceInstanceId == selected.sourceInstanceId
                        let status = isSelected ? "【保留】" : "【移除】"
                        let sourceName = detail.modifier.sourceName
                        Logger.info("  \(index+1). \(status) \(sourceName) - 强度: \(detail.strength)")
                    }
        ...
        if removedDbuffCount > 0 {
            Logger.info("Dbuff去重完成: 原有 \(totalDbuffBefore) 个，去重后 \(totalDbuffAfter) 个，移除了 \(removedDbuffCount) 个重复的")
        } else {
