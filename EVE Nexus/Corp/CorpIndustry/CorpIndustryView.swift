...
class CorpIndustryViewModel: ObservableObject {
    ...
    @Published var hideCompletedAndCancelled = false {
        didSet {
            ...
            if initialLoadDone {
                loadJobs(forceRefresh: true, isFiltering: true)
            }
        }
    }
    ...
    func loadJobs(forceRefresh: Bool = false, isFiltering: Bool = false) {
        // 如果已经加载过且不是强制刷新，则跳过
...
struct CorpIndustryView: View {
    ...
    var body: some View {
            ...
            if viewModel.isLoading {
                ...
                Section(
                    ...
                    let activeJobs = viewModel.jobs.filter { job in
                        (job.status == "active" && job.end_date > viewModel.currentTime)  // 正在进行中
                        || job.status == "ready"  // 已完成但未交付
                        || (job.status == "active" && job.end_date <= viewModel.currentTime)  // 已完成但状态未更新
                    }
        ...
        .refreshable {
            viewModel.loadJobs(forceRefresh: true)
        }
    ...
    var body: some View {
        ...
        .contextMenu {
            ...
            if let locationInfo = locationInfo {
                Button {
                    let locationText = locationInfo.stationName
            ...
            if let installerName = installerName {
                Button {
                    ...
                    Label(
                        String(format: NSLocalizedString("Industry_Copy_Installer_Name", comment: "复制发起人名称: %@"), installerName),
