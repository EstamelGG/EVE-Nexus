...
struct StructureSelectorView: View {
        ...
        List {
            ...
            Section(
                ...
                if isLoading {
                    ...
                    Text(
                        NSLocalizedString(
                            "Structure_Selector_No_Preset_Structures", comment: "暂无预设建筑"
                        )
                    )
    ...
    private func loadDefaultStructures() {
        ...
        guard let url = Bundle.main.url(forResource: "default_structure", withExtension: "json"),
              ...
              let configs = try? JSONDecoder().decode([DefaultStructureConfig].self, from: data)
        else {
    ...
    private func loadCustomStructures() {
        ...
        if !FileManager.default.fileExists(atPath: industryFacilitiesPath.path) {
            try? FileManager.default.createDirectory(
                at: industryFacilitiesPath, withIntermediateDirectories: true
            )
        }
        ...
        do {
            let files = try FileManager.default.contentsOfDirectory(
                at: industryFacilitiesPath, includingPropertiesForKeys: nil
            )
                ...
                for fileURL in jsonFiles {
                    do {
                        ...
                        let config = try JSONDecoder().decode(
                            DefaultStructureConfig.self, from: data
                        )
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                if !structure.isDefault, let systemId = structure.systemId {
                    let systemInfo = getSystemInfo(
                        systemId: systemId, databaseManager: DatabaseManager.shared
                    )
                    HStack(spacing: 4) {
                        ...
                        if let systemName = systemInfo.name {
                            ...
                            Text(
                                NSLocalizedString(
                                    "Structure_Facility_Selector_Unknown_System", comment: "未知星系"
                                )
                            )
                            .font(.caption2)
                            .foregroundColor(.secondary)
                            ...
                            Text(
                                String(
                                    format: NSLocalizedString(
                                        "Structure_Selector_Other_Rigs", comment: "等其他 %d 个"
                                    ),
                                    remainingCount
                                )
                            )
    ...
    var body: some View {
        NavigationStack {
            List {
                        ...
                        HStack(spacing: 12) {
                            ...
                            HStack(spacing: 8) {
                                ...
                                if let systemName = systemInfo.name {
                                    ...
                                    Text(
                                        NSLocalizedString(
                                            "Structure_Info_Unknown_System", comment: "未知星系"
                                        )
                                    )
                                    .font(.body)
                                    .foregroundColor(.primary)
                ...
                if !structure.rigInfos.isEmpty {
                    Section(
                        header: Text(NSLocalizedString("Structure_Info_All_Rigs", comment: "所有插件"))
                    ) {
