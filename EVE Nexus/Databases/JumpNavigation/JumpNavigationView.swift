...
struct JumpSystemData {
    ...
    static func loadAllJumpSystems(databaseManager: DatabaseManager) -> [JumpSystemData] {
        ...
        let query = """
            SELECT u.solarsystem_id, s.solarSystemName, s.solarSystemName_en, 
                   u.system_security, u.x, u.y, u.z
            FROM universe u
            JOIN solarsystems s ON s.solarSystemID = u.solarsystem_id
            WHERE u.system_security < 0.5 -- 排除高安星系，后续还要进一步过滤
            AND u.hasJumpGate -- 排除没有星门的星系，一般是虫洞和GM星系
...
struct JumpNavigationView: View {
    ...
    @State private var selectedShipName: String = ""  // 用于显示飞船名称
    ...
    init(databaseManager: DatabaseManager) {
        ...
        Logger.info("JumpNavigationView初始化完成，已加载飞船数据和\(JumpSystemsCache.shared.systemIdToName.count)个星系名称")
    }
    ...
    var body: some View {
        ...
        .alert(NSLocalizedString("Jump_Navigation_Recalculate_Title", comment: ""), isPresented: $showingConfirmation) {
            Button(NSLocalizedString("Jump_Navigation_Recalculate_Confirm", comment: ""), role: .destructive) {
                recalculateJumpMap()
            }
            ...
            if !pathResults.isEmpty {
                NavigationView {
                    JumpPathResultView(
                        ...
                        shipEnName: ships.values.flatMap({ $0 }).first(where: { $0.id == selectedShip })?.enName ?? "",
                        jdcLevel: JDCLv,
        ...
        VStack {
            List {
                ...
                Section {
                    ...
                    HStack {
                        Spacer()
                        Text("\(NSLocalizedString("Jump_Navigation_Max_Jump_Range", comment: "")): \(String(format: "%.1f Ly", calculateMaxJumpRange()))")
                ...
                Section {
                    ...
                    ForEach(Array(waypointIds.enumerated()), id: \.element) { index, waypointId in
                        HStack {
                            Text(String(format: NSLocalizedString("Jump_Navigation_Waypoint", comment: ""), index + 1))
            ...
            Button(action: {
                ...
                HStack {
                    if progressMessage == NSLocalizedString("Jump_Navigation_Calculating", comment: "") {
                        ProgressView()
                            .scaleEffect(0.8)
                            .padding(.trailing, 8)
                    }
            ...
            .disabled(!isPathCalculationEnabled || progressMessage == NSLocalizedString("Jump_Navigation_Calculating", comment: ""))
            .padding()
        }
    }
    ...
    private func calculatePath() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let allSystems = JumpNavigationHandler.getAllJumpableSystems(databaseManager: self.databaseManager)
            ...
            DispatchQueue.main.async {
                ...
                if let startId = startSystemId {
                    ...
                    Logger.info("路径点: \(waypointIds.map { "\($0)(\(self.getSystemName($0)))" }.joined(separator: ", "))")
                } else {
                ...
                if !avoidSystemIds.isEmpty {
                    Logger.info("避开星系: \(avoidSystemIds.map { "\($0)(\(self.getSystemName($0)))" }.joined(separator: ", "))")
                } else {
                    Logger.info("没有规避星系")
                }
    ...
    private func performPathFinding(
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let systems = JumpNavigationHandler.getAllJumpableSystems(databaseManager: self.databaseManager)
            var systemIdToName: [Int: String] = [:]
            var systemIdToSecurity: [Int: Double] = [:]
            ...
            DispatchQueue.main.async {
                ...
                for (index, result) in results.enumerated() {
                    ...
                    Logger.info("  星系路径: \(result.path.map { "\($0)(\(systemIdToName[$0] ?? "未知"))" }.joined(separator: " -> "))")
                }
    ...
    private func checkAndInitializeJumpMap() {
        ...
        if !fileManager.fileExists(atPath: jumpMapFile.path) {
            Logger.info("跳跃地图文件不存在，开始初始化计算")
            isLoading = true
            progressMessage = NSLocalizedString("Jump_Navigation_Calculating_Jump_Distance", comment: "")
            progressValue = 0.0
            ...
            Logger.info("\(jumpMapFile.path) 文件已存在，跳跃地图已初始化")
        }
    }
    ...
    private func mergePathResults(_ results: [PathResult]) -> PathResult {
        ...
        Logger.info("路径合并完成: 总长度 \(mergedPath.count) 个星系, \(mergedSegments.count) 个路径段, 总距离 \(totalDistance) 光年")
        return mergedResult
    }
...
struct SystemSelectorSheet: View {
    ...
    init(title: String, currentSelection: Int? = nil, onSelect: @escaping (Int) -> Void, onCancel: @escaping () -> Void) {
        self.title = title
        ...
        NavigationView {
            ...
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button(NSLocalizedString("Main_Setting_Cancel", comment: ""), action: {
                        onCancel()
                    })
                }
            }
    ...
    private func loadSystems() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            // 在主线程更新UI
            DispatchQueue.main.async {
                systems = loadedSystems.sorted { $0.name < $1.name }
                isLoading = false
            }
        }
    }
