...
struct SovereigntyListView: View {
                ...
                List {
                    ...
                    if !searchText.isEmpty {
                        ...
                        if !filteredSovereignties.isEmpty {
                            Section(
                                header: Text(
                                    "\(NSLocalizedString("Sovereignty_Search_Results_Factions", comment: "主权势力")) (\(filteredSovereignties.count))"
                                )
                                .fontWeight(.semibold)
                        ...
                        if !matchedSystems.isEmpty {
                            Section(
                                header: Text(
                                    "\(NSLocalizedString("Sovereignty_Search_Results_Systems", comment: "星系")) (\(matchedSystems.count))"
                                )
                                .fontWeight(.semibold)
                                .foregroundColor(.primary)
                                .textCase(.none)
    ...
    private func loadSystemSovereigntyInfo(sovereigntyData: [SovereigntyData]) {
        ...
        Logger.info(
            "星系主权图标加载：复用现有联盟 \(existingAllianceIds.count) 个，新加载联盟 \(newAllianceIds.count) 个；复用现有派系 \(existingFactionIds.count) 个，新加载派系 \(newFactionIds.count) 个"
        )
    }
