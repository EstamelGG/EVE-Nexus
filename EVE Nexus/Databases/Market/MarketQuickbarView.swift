        ...
        MarketItemSelectorBaseView(
            ...
            searchQuery: { _ in
                ...
                if let typeIDs = allowTypeIDs, !typeIDs.isEmpty {
                    ...
                    return "t.marketGroupID IN (\(groupIDsString)) AND t.type_id IN (\(typeIDsString)) AND (t.name LIKE ? OR t.en_name LIKE ?)"
                } else {
                    // 否则只筛选市场分组
...
struct MarketItemSelectorIntegratedView: View {
        ...
        MarketItemSelectorBaseView(
            ...
            searchQuery: { text in
                ...
                if let typeIDs = allowTypeIDs, !typeIDs.isEmpty {
                    // 如果有物品ID白名单，添加物品ID筛选条件
                    let typeIDsString = typeIDs.map { String($0) }.joined(separator: ",")
                    return "t.marketGroupID IN (\(groupIDsString)) AND t.type_id IN (\(typeIDsString)) AND (t.name LIKE ? OR t.en_name LIKE ? OR t.type_id = ?)"
                } else {
                    // 否则只筛选市场分组
    ...
    private func getFilteredRootGroups() -> [MarketGroup] {
        ...
        if allowedMarketGroups.isEmpty {
            ...
            initialRootGroups = MarketManager.shared.setRootGroups(marketGroups, allowedIDs: allowedMarketGroups)
        }
        
        // 如果有物品ID白名单且有效组ID已计算，过滤出有效的根目录
        let validRootGroups: [MarketGroup]
        if let _ = allowTypeIDs, !validMarketGroups.isEmpty {
