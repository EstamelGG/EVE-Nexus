...
struct MarketQuickbar: Identifiable, Codable {
    ...
    init(
        ...
        marketLocation = "region_id:\(regionID)" // 设置默认的市场位置字符串
    }
...
struct MarketItemSelectorBaseView<Content: View>: View {
    ...
    var groupedSearchResults: [(id: Int, name: String, items: [DatabaseListItem])] {
        ...
        let remainingCategories = groupedByCategory.keys.filter { !categoryPriority.contains($0) }
            .sorted()
...
struct MarketQuickbarDetailView: View {
    ...
    @State private var isShowingClipboardAlert = false // 新增：控制剪贴板导入提示
...
class MarketClipboardParser {
        ...
        -> [String: Int]
    {
        ...
        // 优化的SQL查询，支持多语言搜索
        let query = """
        SELECT type_id, name, en_name, zh_name
        FROM types
        WHERE name IN (\(placeholders))
           OR en_name IN (\(placeholders))
           OR zh_name IN (\(placeholders))
        """

        // 创建参数数组（每个名称重复3次，分别对应name、en_name、zh_name）
        let parameters = names + names + names
        ...
        if case let .success(rows) = databaseManager.executeQuery(query, parameters: parameters) {
            for row in rows {
                if let typeID = row["type_id"] as? Int {
                    ...
                    // 为所有匹配的名称建立映射
                    for name in names {
                        if name.caseInsensitiveCompare(itemName) == .orderedSame
                            || name.caseInsensitiveCompare(enName) == .orderedSame
                            || name.caseInsensitiveCompare(zhName) == .orderedSame
                        {
