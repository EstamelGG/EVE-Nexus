...
class PlanetaryResourceCalculator {
            ...
            if case let .success(rows) = databaseManager.executeQuery(query) {
                for row in rows {
                    ...
                    guard let inputTypeIdsStr = row["input_typeid"] as? String,
                          let inputValuesStr = row["input_value"] as? String else {
                        continue
                    }
        ...
        if !baseResourcesWithDepth.isEmpty {
            ...
            if case let .success(nameRows) = databaseManager.executeQuery(nameQuery) {
                ...
                for resource in baseResourcesWithDepth {
                    results.append(PlanetaryResourceResult(
                ...
                for result in results.sorted(by: { $0.depth < $1.depth }) {
                    Logger.info("资源: \(result.name) (ID: \(result.typeId)), 数量: \(String(format: "%.2f", result.quantity)), 深度: \(result.depth)")
                }
        ...
        if case let .success(rows) = databaseManager.executeQuery(query) {
            ...
            for row in rows {
                guard let resourceId = row["resource_id"] as? Int,
                      ...
                      let planetTypeId = (row["planet_type_id"] as? Double).map({ Int($0) }) ?? (row["planet_type_id"] as? Int),
                      let planetName = row["planet_name"] as? String else {
                    continue
                }
                ...
                if currentResourceId != resourceId {
                    if let id = currentResourceId, let name = currentResourceName {
                        results.append(ResourcePlanetResult(
                            resourceId: id,
                            resourceName: name,
                ...
                currentPlanets.append((id: planetTypeId, name: planetName, iconFileName: iconFileName))
            }
    ...
    func calculateSystemScores(
        ...
        Logger.debug("需要的资源列表: \(requiredResources.map { "\($0.name)(ID: \($0.typeId))" }.joined(separator: ", "))")
        ...
        for result in resourcePlanets {
            ...
            Logger.debug("资源 \(result.resourceName)(ID: \(result.resourceId)) 可在以下行星类型生产: \(result.availablePlanets.map { $0.name }.joined(separator: ", "))")
        }
        ...
        for row in rows {
            guard let systemId = row["solarsystem_id"] as? Int,
                  ...
                  let security = row["system_security"] as? Double else {
                continue
            }
            
            processedSystems += 1
            Logger.debug("-------------------------------------------")
            Logger.debug("评估星系: \(systemName) (ID: \(systemId), 地区: \(regionName), 安全等级: \(String(format: "%.1f", security)))")
            ...
            for (planetType, columnName) in PlanetaryUtils.planetTypeToColumn {
                if let count = row[columnName] as? Int, count > 0 {
                    let planetName = resourcePlanets.flatMap { $0.availablePlanets }.first { $0.id == planetType }?.name ?? "未知行星"
                    planetTypeCounts[planetName] = count
                }
            }
            Logger.debug("星系包含行星类型: \(planetTypeCounts.map { "\($0.key): \($0.value)颗" }.joined(separator: ", "))")
                                ...
                                if let planetTypes = resourceToPlanetTypes[resourceId] {
                                    ...
                                    if planetCount > 0 {
                                        ...
                                        if let jumps = systemJumps[neighborId], jumps <= maxJumps {
                                            ...
                                            // 获取资源名称
                                            let resourceName = requiredResources.first(where: { $0.typeId == resourceId })?.name ?? "未知资源"
                                            Logger.debug("资源 \(resourceName) 在距离 \(jumps) 跳的邻近星系 \(neighborId) 中找到")
                                        } else if let jumps = systemJumps[neighborId] {
                                            Logger.debug("忽略距离超出限制的系统 \(neighborId) 距离为 \(jumps) 跳，最大允许跳数: \(maxJumps)")
                                        }
                                    }
                                }
            ...
            if !missingResources.isEmpty {
                Logger.debug("星系 \(systemName) 不满足条件: 即使在 \(maxJumps) 跳范围内仍然缺少 \(missingResources.count) 种资源")
                continue
            }
            ...
            // 根据达成这个条件的资源比例给予奖励分数
            if !availableResources.isEmpty {
                let balanceRatio = Double(resourcesWithAtLeastTwoPlanets) / Double(availableResources.count)
                let balanceScore = balanceRatio * 100
                score += balanceScore
                Logger.debug("均衡性评分: \(resourcesWithAtLeastTwoPlanets)/\(availableResources.count) 资源有至少2个星球, 均衡率 \(String(format: "%.2f", balanceRatio)), 得分 +\(String(format: "%.2f", balanceScore))")
            ...
            for (neighborId, info) in additionalResources {
                ...
                // 获取资源名称用于日志
                let resourceName = requiredResources.first(where: { $0.typeId == info.resourceId })?.name ?? "未知资源"
                Logger.debug("相邻资源: \(resourceName) 在星系 \(neighborId) 距离 \(info.jumps) 跳, 基础分 \(baseNeighborScore), 跳数惩罚 -\(jumpPenalty), 最终得分 \(resourceNeighborScore)")
            }
            score += neighborScore
            
            // 4. 如果所有资源都可本地获取，无需跳转其他星系，给予显著奖励
            if !availableResources.isEmpty && additionalResources.isEmpty && missingResources.isEmpty {
        ...
        if sortedResults.isEmpty {
            ...
            Logger.debug("最高评分星系: \(sortedResults[0].systemName)，得分: \(String(format: "%.2f", sortedResults[0].score))")
        }
