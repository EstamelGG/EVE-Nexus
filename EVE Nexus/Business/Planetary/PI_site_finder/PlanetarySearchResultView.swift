...
struct PlanetarySearchResultView: View {
            ...
            if results.isEmpty {
                ...
                Section {
                    VStack(spacing: 16) {
                        ...
                        Text(NSLocalizedString("Planetary_Search_No_Results", comment: "没有找到能够生产所需资源的星系"))
                            .font(.headline)
                            .multilineTextAlignment(.center)
                        
                        Text(NSLocalizedString("Planetary_Search_No_Results_Tips", comment: "请考虑以下调整方案:\n• 增加星系跳数范围\n• 选择其他星域或主权\n• 选择其他可能更容易生产的产品"))
                            .font(.subheadline)
                    ...
                    Section {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            if !result.availableResources.isEmpty {
                                ...
                                ForEach(Array(result.availableResources.keys).sorted(), id: \.self) { resourceId in
                                    if let count = result.availableResources[resourceId] {
                                        HStack {
                                            // 添加资源图标
                                            if let iconFileName = resourceInfo[resourceId]?.iconFileName {
                                            ...
                                            Text("- \(resourceInfo[resourceId]?.name ?? "未知资源"): \(String(format: NSLocalizedString("Planetary_Resource_Planet_Count", comment: ""), "\(count)"))")
                                                .font(.subheadline)
                                        }
                                    }
                                }
                            ...
                            if !result.additionalResources.isEmpty {
                                ...
                                // 分组并排序后显示
                                let groupedResources = groupAdditionalResourcesByType(result.additionalResources)
                                ForEach(groupedResources, id: \.resourceId) { group in
                                    VStack(alignment: .leading) {
                                        ...
                                        // 显示提供该资源的所有星系
                                        ForEach(group.systems.sorted(by: { $0.jumps < $1.jumps }), id: \.systemId) { systemInfo in
                                            let systemName = systemNames[systemInfo.systemId] ?? "\(systemInfo.systemId)"
                                            Text("  • \(String(format: NSLocalizedString("Planetary_Resource_System_Format", comment: ""), systemName, systemInfo.jumps))")
                                                .font(.caption)
                                                .padding(.leading, 8)
                                        }
                                    }
                            ...
                            Text(String(format: NSLocalizedString("Planetary_Coverage", comment: ""), result.coverage))
                                .font(.subheadline)
                                .foregroundColor(.gray)
                        }
                        .padding(.top, 4)
            ...
            for row in rows {
                if let typeId = row["type_id"] as? Int,
                   ...
                   let iconFileName = row["icon_filename"] as? String {
                    tempInfo[typeId] = (name: name, iconFileName: iconFileName.isEmpty ? "not_found" : iconFileName)
                }
            }
    ...
    private func groupAdditionalResourcesByType(_ additionalResources: [Int: (resourceId: Int, jumps: Int)]) -> [(resourceId: Int, systems: [(systemId: Int, jumps: Int)])] {
