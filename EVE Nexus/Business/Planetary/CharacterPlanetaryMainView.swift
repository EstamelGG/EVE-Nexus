...
final class CharacterPlanetaryViewModel: ObservableObject {
    ...
    func getPlanetTypeInfo(for planetType: String) -> PlanetTypeInfo? {
        ...
        if let typeId = PlanetaryUtils.columnToPlanetType[planetType], let info = planetTypeInfo[typeId] {
            return info
        }
        return nil
    }
            ...
            if viewModel.isLoading {
                ...
                if characterId != nil {
                    if viewModel.planets.isEmpty {
                        ...
                        Section(header: Text(NSLocalizedString("Main_Planetary_of_Mine", comment: "")), footer: Text(String(format: NSLocalizedString("Main_Planetary_Total_Count", comment: ""), viewModel.planets.count))) {
