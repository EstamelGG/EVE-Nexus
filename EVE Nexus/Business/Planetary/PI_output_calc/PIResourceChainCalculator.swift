...
class PIResourceChainCalculator {
    ...
    // 一次性计算完整的资源链，包括所有层级
    func calculateFullResourceChain(for resourceId: Int, in systemIds: [Int], completion: @escaping ([PIResourceChainInfo]?) -> Void) {
        // 首先获取资源的基本信息
        guard let resourceInfo = resourceCache.getResourceInfo(for: resourceId),
              let resourceLevel = resourceCache.getResourceLevel(for: resourceId) else {
    ...
    private func recursivelyGetAllResources(
        ...
        for requiredId in requiredResourceIds {
            ...
            guard let resourceInfo = resourceCache.getResourceInfo(for: requiredId),
                  let resourceLevel = resourceCache.getResourceLevel(for: requiredId) else {
                continue
            }
    ...
    private func loadPlanetTypesForResources(_ resourceIds: [Int]) {
        ...
        if case let .success(rows) = databaseManager.executeQuery(query) {
            ...
            for row in rows {
                if let resourceId = (row["resource_id"] as? Double).map({ Int($0) }) ?? (row["resource_id"] as? Int),
                   let planetTypesStr = row["planet_types"] as? String {
                    // 将字符串按逗号分割，并处理每个数值
                    let planetTypes = planetTypesStr
                        .split(separator: ",")
    ...
    // 检查资源在指定星系中是否可以生产
    private func checkResourceAvailability(in systemIds: [Int], resourceChain: inout [PIResourceChainInfo]) {
        // 查询指定星系中的行星类型
        let planetQuery = """
            SELECT 
                solarsystem_id,
        ...
        for i in 0..<resourceChain.count {
            ...
            // 检查是否所有所需行星类型都可用
            let allRequiredPlanetTypesAvailable = !requiredPlanetTypes.isEmpty && requiredPlanetTypes.isSubset(of: allAvailablePlanetTypes)
