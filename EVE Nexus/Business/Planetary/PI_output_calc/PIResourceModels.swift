...
class PIResourceCache {
    ...
    // 行星类型到列名映射
    private var planetTypeToColumnCache: [Int: String] = [:]
    ...
    // 获取资源信息
    func getResourceInfo(for resourceId: Int) -> (name: String, iconFileName: String, marketGroupId: Int)? {
    ...
    // 获取资源配方
    func getSchematic(for resourceId: Int) -> (outputValue: Int, inputTypeIds: [Int], inputValues: [Int])? {
            ...
            for row in rows {
                if let outputTypeId = row["output_typeid"] as? Int,
                   ...
                   let inputValueStr = row["input_value"] as? String {
                    ...
                    // 解析输入资源ID和值
                    let inputTypeIds = inputTypeIdStr.components(separatedBy: ",").compactMap { Int($0.trimmingCharacters(in: .whitespaces)) }
                    let inputValues = inputValueStr.components(separatedBy: ",").compactMap { Int($0.trimmingCharacters(in: .whitespaces)) }
