...
struct PIOutputCalculatorView: View {
    ...
    var body: some View {
        List {
            Section(header: Text(NSLocalizedString("PI_Output_Filter_Conditions", comment: ""))) {
                Button(action: {
                    ...
                    HStack {
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text(NSLocalizedString("Planetary_System_Description", comment: "选择作为生产中心的星系"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                        Spacer()
                        Text(selectedSolarSystemName.isEmpty ? 
                ...
                Picker(selection: $maxJumps) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(NSLocalizedString("PI_Output_Max_Jumps", comment: ""))
                            .foregroundColor(.primary)
                        Text(NSLocalizedString("Planetary_Jump_Description", comment: "最多允许在几跳范围内收集资源"))
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                }
                ...
                if maxJumps > 0 {
                    Button {
                        ...
                        HStack {
                            VStack(alignment: .leading, spacing: 2) {
                                Text(NSLocalizedString("Main_Planetary_Select_Sovereignty", comment: "选择主权"))
                                    .foregroundColor(.primary)
                                Text(NSLocalizedString("Planetary_Sovereignty_Description", comment: "要在哪个主权辖区生产"))
                                    .font(.caption)
                                    .foregroundColor(.secondary)
                            }
                            Spacer()
            ...
            if isCalculating {
                ...
                Section {
                    NavigationLink(destination: SystemsListView(
                    ...
                    NavigationLink(destination: PlanetTypesSummaryView(
                        systemIds: Array(systemsInRange)
                    )) {
                ...
                Section(header: Text(NSLocalizedString("PI_Output_Resources", comment: "可用资源"))) {
                    // 检查是否有资源正在加载
                    if isLoadingP0Resources || isLoadingP1Resources || isLoadingP2Resources || isLoadingP3Resources || isLoadingP4Resources {
                        HStack {
                            Spacer()
                            ProgressView()
                            Text(NSLocalizedString("PI_Output_Loading_Resources", comment: "加载资源中..."))
                                .foregroundColor(.gray)
                                .padding(.leading, 8)
                            Spacer()
                        }
                        ...
                        if !p0Resources.isEmpty {
                            NavigationLink(destination: PIResourcesListView(
                                title: "P0",
                                ...
                                HStack {
                                    Text("P0")
                                    Spacer()
                                    Text("\(p0Resources.count) \(NSLocalizedString("Types", comment: "种"))")
                                        .foregroundColor(.gray)
                                }
                            }
                        }
                        ...
                        if !availableP1Resources.isEmpty {
                            NavigationLink(destination: PIResourcesListView(
                                ...
                                HStack {
                                    Text("P1")
                                    Spacer()
                                    Text("\(availableP1Resources.count) \(NSLocalizedString("Types", comment: "种"))")
                                        .foregroundColor(.gray)
                                }
                            }
                        }
                        ...
                        if !availableP2Resources.isEmpty {
                            NavigationLink(destination: PIResourcesListView(
                                ...
                                HStack {
                                    Text("P2")
                                    Spacer()
                                    Text("\(availableP2Resources.count) \(NSLocalizedString("Types", comment: "种"))")
                                        .foregroundColor(.gray)
                                }
                            }
                        }
                        ...
                        if !availableP3Resources.isEmpty {
                            NavigationLink(destination: PIResourcesListView(
                                ...
                                HStack {
                                    Text("P3")
                                    Spacer()
                                    Text("\(availableP3Resources.count) \(NSLocalizedString("Types", comment: "种"))")
                                        .foregroundColor(.gray)
                                }
                            }
                        }
                        ...
                        if !availableP4Resources.isEmpty {
                            NavigationLink(destination: PIResourcesListView(
                                ...
                                HStack {
                                    Text("P4")
                                    Spacer()
                                    Text("\(availableP4Resources.count) \(NSLocalizedString("Types", comment: "种"))")
                                        .foregroundColor(.gray)
                                }
                            }
                        }
                        ...
                        // 如果没有任何可用资源，显示提示信息
                        if p0Resources.isEmpty && 
                           p1Resources.filter({ $0.canProduce }).isEmpty && 
    ...
    private func calculateSystemsInRange() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            do {
                ...
                guard let starMap = try JSONSerialization.jsonObject(with: data) as? [String: [Int]] else {
                        ...
                        if let neighbors = starMap[String(current.systemId)] {
                            for neighbor in neighbors {
                                if !visited.contains(neighbor) {
                                    ...
                                    queue.append((systemId: neighbor, distance: current.distance + 1))
                                }
                            }
                        }
                    }
                ...
                if let sovereigntyId = selectedSovereigntyID {
                    ...
                    Task {
                        do {
                            // 获取主权数据
                            let sovereigntyData = try await SovereigntyDataAPI.shared.fetchSovereigntyData(forceRefresh: false)
                            
                            // 根据主权ID筛选星系
                            let systemsUnderSovereignty = Set(sovereigntyData.compactMap { data -> Int? in
                            ...
                            let filteredResult = capturedResult.intersection(systemsUnderSovereignty)
                            
                            // 更新主线程UI
                            await MainActor.run {
                                systemsInRange = filteredResult
                                isCalculating = false
                                Logger.info("根据主权筛选计算完成，在\(maxJumps)跳范围内找到\(capturedResult.count)个星系，其中\(filteredResult.count)个星系属于指定主权")
                            ...
                            await MainActor.run {
                                ...
                                isCalculating = false
                                Logger.info("获取主权数据失败，计算完成，在\(maxJumps)跳范围内找到\(capturedResult.count)个星系")
    ...
    private func loadResources(for systemIds: [Int]) {
        ...
        loadPIResources(level: .p0, for: systemIds, availablePIIds: Set<Int>()) { availableP0Ids in
            ...
            if !availableP0Ids.isEmpty {
                // P0资源加载完成后，加载P1资源信息
                loadPIResources(level: .p1, for: systemIds, availablePIIds: availableP0Ids) { availableP1Ids in
                    // 合并P0和P1的可用资源
                    ...
                    if !availableP1Ids.isEmpty {
                        // P1资源加载完成后，加载P2资源信息
                        loadPIResources(level: .p2, for: systemIds, availablePIIds: availableP0P1Ids) { availableP2Ids in
                            // 合并P0、P1和P2的可用资源
                            ...
                            if !availableP2Ids.isEmpty {
                                // P2资源加载完成后，加载P3资源信息
                                loadPIResources(level: .p3, for: systemIds, availablePIIds: availableP0P1P2Ids) { availableP3Ids in
                                    // 合并P0、P1、P2和P3的可用资源
                                    let availableP0P1P2P3Ids = availableP0P1P2Ids.union(availableP3Ids)
                                    
                                    // 只有在有可用P3资源的情况下才加载P4资源
                                    if !availableP3Ids.isEmpty {
                                        // P3资源加载完成后，加载P4资源信息
                                        loadPIResources(level: .p4, for: systemIds, availablePIIds: availableP0P1P2P3Ids) { _ in }
                                    } else {
    ...
    // 统一的资源加载方法
    private func loadPIResources(level: PIResourceLevel, for systemIds: [Int], availablePIIds: Set<Int>, completion: @escaping (Set<Int>) -> Void) {
        ...
        DispatchQueue.main.async {
            switch level {
            case .p0: isLoadingP0Resources = true; p0Resources.removeAll()
            case .p1: isLoadingP1Resources = true; p1Resources.removeAll()
            case .p2: isLoadingP2Resources = true; p2Resources.removeAll()
            case .p3: isLoadingP3Resources = true; p3Resources.removeAll()
            case .p4: isLoadingP4Resources = true; p4Resources.removeAll()
            }
        }
            ...
            if !resourceIds.isEmpty {
                ...
                for (resourceId, info) in resourceInfo {
                    if let requiredInputIds = resourceToInputMap[resourceId] {
                        // 检查是否所有需要的输入资源都可用
                        let canProduce = !requiredInputIds.isEmpty && requiredInputIds.isSubset(of: availablePIIds)
                        
                        // 只添加可以生产的资源
                        if canProduce {
                            switch level {
                            case .p1:
                                resourceInfos.append(P1ResourceInfo(
                            ...
                            case .p2:
                                resourceInfos.append(P2ResourceInfo(
                                    resourceId: resourceId,
                                    resourceName: info.name,
                                    iconFileName: info.iconFileName,
                            ...
                            case .p3:
                                resourceInfos.append(P3ResourceInfo(
                                    resourceId: resourceId,
                                    resourceName: info.name,
                                    iconFileName: info.iconFileName,
                            ...
                            case .p4:
                                resourceInfos.append(P4ResourceInfo(
                                    resourceId: resourceId,
                                    resourceName: info.name,
                                    iconFileName: info.iconFileName,
                ...
                case .p1:
                    ...
                    let sortedResources = typedResources.sorted { $0.resourceName < $1.resourceName }
                    DispatchQueue.main.async {
    ...
    private func loadP0ResourcesImpl(for systemIds: [Int], completion: @escaping (Set<Int>) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            if case let .success(resourceRows) = DatabaseManager.shared.executeQuery(p0ResourceQuery) {
            ...
            if !resourceIds.isEmpty {
                // 获取资源可用的行星类型
                let resourceCalculator = PlanetaryResourceCalculator(databaseManager: DatabaseManager.shared)
                ...
                for row in rows {
                    ...
                    for resourceId in resourceIds {
                        if let planetTypes = resourceToPlanetTypes[resourceId] {
                            ...
                            // 更新资源可用行星总数
                            if planetCount > 0 {
                                resourcePlanetCounts[resourceId] = (resourcePlanetCounts[resourceId] ?? 0) + planetCount
                            }
                        }
                    }
                }
                ...
                for (resourceId, count) in resourcePlanetCounts {
                    if let info = resourceInfo[resourceId] {
                        p0ResourceInfos.append(P0ResourceInfo(
                            resourceId: resourceId,
                            resourceName: info.name,
