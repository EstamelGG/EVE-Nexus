...
struct PIResourceChainView: View {
    ...
    var body: some View {
        List {
            if isLoading {
                ...
                // 基本信息部分
                    Section(header: Text(NSLocalizedString("PI_Production_Plan_BasicInfo", comment: ""))) {
                    // 产品信息
                    HStack {
                    ...
                    HStack {
                        ...
                        Text("\(maxJumps) \(NSLocalizedString("PI_Production_Plan_Systems", comment: "跳"))")
                            .foregroundColor(.secondary)
                    }
                    
                    // 主权信息
                    if let sovereignty = sovereigntyInfo, let icon = sovereignty.icon, let name = sovereignty.name {
                        HStack {
                ...
                if !p0Resources.isEmpty {
                    Section(header: Text(NSLocalizedString("PI_Production_Plan_P0Requirements", comment: "需求资源与比例"))) {
                        ForEach(p0Resources, id: \.id) { resource in
                            NavigationLink(destination: P0ResourceDetailView(resourceId: resource.id, resourceName: resource.name, systemIds: systemIds)) {
                                HStack {
                                    Image(uiImage: IconManager.shared.loadUIImage(for: resource.icon))
                                        .resizable()
                                        .scaledToFit()
                                        .frame(width: 32, height: 32)
                                        .cornerRadius(4)
        ...
        calculator.calculateFullResourceChain(for: resourceId, in: systemIds) { result in
            DispatchQueue.main.async {
                if let allResources = result, !allResources.isEmpty {
                    ...
                    self.errorMessage = NSLocalizedString("PI_Resource_Chain_Error", comment: "无法加载资源链信息") + " (ID: \(resourceId))"
                }
    ...
    private func loadSystemInfo() {
        ...
        if let systemInfo = PIResourceCache.shared.getSystemInfo(for: systemId) {
            self.systemInfo = (name: systemInfo.name, security: systemInfo.security, region: systemInfo.region)
        }
    }
            ...
            do {
                ...
                if let systemData = sovereigntyData.first(where: { $0.systemId == systemId }) {
                    ...
                    if let allianceId = systemData.allianceId {
                        let allianceInfo = try await AllianceAPI.shared.fetchAllianceInfo(allianceId: allianceId)
            ...
            if let resources = resourcesByLevel[level] {
                for resource in resources {
                    ...
                    for upper in upperResources {
                        if let schematic = PIResourceCache.shared.getSchematic(for: upper.resourceId) {
                            let outputValue = Double(schematic.outputValue)
                            let inputIndex = schematic.inputTypeIds.firstIndex(of: resource.resourceId) ?? -1
                            if inputIndex >= 0 {
