...
class StructureMarketManager {
    ...
    private func fetchStructureMarketOrdersFromAPI(
        ...
        // 构建API URL
        guard let url = URL(string: "https://esi.evetech.net/markets/structures/\(structureId)/")
        else {
    ...
    private func loadOrdersFromFile(structureId: Int64) async throws -> [StructureMarketOrder] {
        ...
        guard FileManager.default.fileExists(atPath: filePath.path) else {
            throw NSError(
                domain: "StructureMarketManager", code: 404,
                userInfo: [NSLocalizedDescriptionKey: "本地订单文件不存在"]
            )
        }
