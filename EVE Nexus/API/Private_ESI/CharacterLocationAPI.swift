...
class CharacterLocationAPI {
    ...
    // 获取角色在线状态
    func fetchCharacterOnlineStatus(characterId: Int, forceRefresh: Bool = false) async throws -> CharacterOnlineStatus {
        // 如果不是强制刷新，先尝试使用缓存
        if !forceRefresh {
            // 1. 先检查内存缓存
    ...
    private func getOnlineStatusDiskCache(characterId: Int) -> OnlineStatusCacheEntry? {
        ...
        guard let data = UserDefaults.standard.data(forKey: key),
              let cache = try? JSONDecoder().decode(OnlineStatusCacheEntry.self, from: data) else {
            return nil
        }
        return cache
    }
