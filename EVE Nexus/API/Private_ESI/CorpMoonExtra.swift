...
public struct MoonExtractionInfo: Codable {
    ...
    public init(chunk_arrival_time: String, extraction_start_time: String, moon_id: Int64, natural_decay_time: String, structure_id: Int64) {
...
public class CorpMoonExtractionAPI {
    ...
    private func fetchFromAPI(corporationId: Int, characterId: Int) async throws -> [MoonExtractionInfo] {
        ...
        while hasMorePages {
            ...
            let urlString = "https://esi.evetech.net/latest/corporation/\(corporationId)/mining/extractions/?datasource=tranquility&page=\(currentPage)"
            ...
            do {
                ...
                Logger.error("获取月矿提取信息失败 - 军团ID: \(corporationId), 页码: \(currentPage), 错误: \(error)")
                throw error
            }
        }
    ...
    private func getCacheDirectory() -> URL? {
        ...
        let cacheDirectory = documentsDirectory.appendingPathComponent("CorpMoon", isDirectory: true)
