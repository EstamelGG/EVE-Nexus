...
class CorporationAPI {
    ...
    /// 更新军团信息中的名称
    private func updateCorporationInfoName(info: CorporationInfo, newName: String)
        -> CorporationInfo
    {
    ...
    /// 获取军团联盟历史
    func fetchAllianceHistory(corporationId: Int, forceRefresh: Bool = false) async throws
        -> [CorporationAllianceHistory]
    {
        ...
        // 从网络获取数据
        let urlString =
            "https://esi.evetech.net/corporations/\(corporationId)/alliancehistory/?datasource=tranquility"
        guard let url = URL(string: urlString) else {
            throw NetworkError.invalidURL
        }
